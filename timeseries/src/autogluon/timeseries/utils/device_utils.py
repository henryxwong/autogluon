...
def get_device(preferred_backend: Literal["cuda", "mps", "xpu", "cpu"] | None = None) -> str:
    ...
    if preferred_backend is not None:
        if preferred_backend == "cuda":
            if torch.cuda.is_available():
                logger.debug("Using CUDA device (preferred backend)")
                return "cuda"
            else:
                raise ValueError(
            ...
            if torch.backends.mps.is_available():
                logger.debug("Using MPS device (preferred backend)")
                return "mps"
            else:
                raise ValueError(
            ...
            if torch.xpu.is_available():
                logger.debug("Using XPU device (preferred backend)")
                return "xpu"
            else:
                raise ValueError(
            ...
            logger.debug("Using CPU device (preferred backend)")
            return "cpu"
        else:
            raise ValueError(
    ...
    if torch.cuda.is_available():
        ...
        if is_rocm:
            logger.debug("Using ROCm device (AMD GPU)")
        else:
            logger.debug("Using CUDA device (NVIDIA GPU)")
        return "cuda"
    elif torch.xpu.is_available():
        logger.debug("Using XPU device (Intel GPU)")
        return "xpu"
    elif torch.backends.mps.is_available():
        logger.debug("Using MPS device (Apple Silicon)")
        return "mps"
    else:
        logger.debug("No GPU detected, using CPU device")
        return "cpu"
